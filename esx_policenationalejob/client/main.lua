...
function setUniform(job, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			...
			if job == 'bullet3_wear' then
				SetPedArmour(playerPed, 100)
			end

			if job == 'bullet4_wear' then
				SetPedArmour(playerPed, 100)
			end
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = 'Tenue BAC', value = 'tenu_ma' },
		{ label = 'Tenue Motard', value = 'tenu_doag' },
		{ label = 'Tenue CRS', value = 'tenu_hiv'},
		{ label = _U('bullet3_wear'), value = 'bullet3_wear' },
		{ label = _U('bullet4_wear'), value = 'bullet4_wear' },
		{ label = _U('bullet_wear'), value = 'bullet_wear' },
		{ label = _U('bullet2_wear'), value = 'bullet2_wear' },
	}
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'pn')

						TriggerServerEvent('esx_service:disableService', 'pn')
						TriggerEvent('esx_pnjob:updateBlip')
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				shopCoords = Config.PnStations[station].Vehicles[partNum].InsideShop
			...
			elseif type == 'helicopter' then
				shopCoords = Config.PnStations[station].Helicopters[partNum].InsideShop
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_pnjob:storeNearbyVehicle', function(storeSuccess, foundNum)
...
function GetAvailableVehicleSpawnPoint(station, part, partNum)
	local spawnPoints = Config.PnStations[station][part][partNum].SpawnPoints
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_pnjob:buyJobVehicle', function (bought)
					if bought then
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pn_actions', {
		css      = 'job',
		title    = 'PN',
		align    = 'top-left',
		elements = {
		...
		if data.current.value == 'citizen_interaction' then
			...
			if PlayerData.job and PlayerData.job.name == 'pn' and PlayerData.job.grade_name == 'chef' or PlayerData.job.grade_name == 'boss' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_pnjob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
					...
					elseif action == 'handcuff' then
						--TriggerServerEvent('esx_pnjob:handcuff', GetPlayerServerId(closestPlayer))
						...
						if distance <= 2.0 then
							TriggerServerEvent('esx_pnjob:requestarrest', target_id, playerheading, playerCoords, playerlocation)
							Wait(5000)
							TriggerServerEvent('esx_pnjob:handcuff', GetPlayerServerId(closestPlayer))
						else
					...
					elseif action == 'handcuff2' then
						...
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						local target_id = GetPlayerServerId(target)
						TriggerServerEvent('esx_pnjob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
						Wait(5000)
						TriggerServerEvent('esx_pnjob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_pnjob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_pnjob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_pnjob:OutVehicle', GetPlayerServerId(closestPlayer))
					...
					if amount == nil or amount <= 0 then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							Citizen.CreateThread(function()
								...
								TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(closestPlayer), 'society_pn', 'pn', amount)
							end)
						end
					end
				end,
		...
		elseif data.current.value == 'status' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'status_service', {
				...
				if action == 'prise' then
					local info = 'prise'
					TriggerServerEvent('pn:PriseEtFinservice', info)
		...
		elseif data.current.value == 'lspd_equipement' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lspd_equipement', {
				...
				if action == 'equipement_cadet' then
					...
					GiveWeaponToPed(PlayerPedId(), "weapon_flashlight", 1, false, false)
				...
				elseif action == 'equipement_officier' then
					...
					GiveWeaponToPed(PlayerPedId(), "WEAPON_NIGHTSTICK", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "weapon_flashlight", 1, false, false)
                                        GiveWeaponToPed(PlayerPedId(), "weapon_lgcougar", 255, false, false)
                                        GiveWeaponToPed(PlayerPedId(), "weapon_lbd", 255, false, false)
                                        GiveWeaponToPed(PlayerPedId(), "weapon_lbd2", 255, false, false)
				...
				elseif action == 'equipement_lieutenant' then
					...
					ESX.ShowAdvancedNotification('PN INFO', 'Armurie PN', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
					PlaySoundFrontend(-1, "Bus_Schedule_Pickup", "DLC_PRISON_BREAK_HEIST_SOUNDS", 0)
				elseif action == 'equipement_cmd' then
					...
					GiveWeaponToPed(PlayerPedId(), "WEAPON_STUNGUN", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_NIGHTSTICK", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "weapon_specialcarbine", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "weapon_flashlight", 1, false, false)
                                        GiveWeaponToPed(PlayerPedId(), "weapon_smg", 255, false, false)
                                        GiveWeaponToPed(PlayerPedId(), "weapon_lgcougar", 255, false, false)
                                        GiveWeaponToPed(PlayerPedId(), "weapon_lbd", 255, false, false)
					...
					ESX.ShowAdvancedNotification('PN', 'Armurie PN', 'Voila ton équipement, le perd pas.', 'CHAR_AMMUNATION', 1)
					PlaySoundFrontend(-1, "Bus_Schedule_Pickup", "DLC_PRISON_BREAK_HEIST_SOUNDS", 0)
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = _U('balise'), model = 'prop_mp_num_0'},
					{label = _U('routebarrer'), model = 'prop_mp_barrier_02'},
					{label = _U('droite'), model = 'prop_mp_num_1'},
					{label = _U('gauche'), model = 'prop_mp_num_2'},
					{label = _U('interdit'), model = 'prop_mp_num_4'},
					{label = _U('inondation'), model = 'prop_mp_num_5'},
					{label = _U('barriere'), model = 'prop_mp_barrier_02b'},
				...
				if data2.current.model == 'prop_roadcone02b' then
					z = z + 2.0
				end
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_pnjob:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('esx_pnjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_pnjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_pnjob:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(player), 'society_pn', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_pnjob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_pnjob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_pnjob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_pnjob:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_pnjob:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('esx_pnjob:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('esx_pnjob:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
--			end
--		else
--			ESX.TriggerServerCallback('esx_pnjob:buyWeapon', function(bought)
--				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_pnjob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_pnjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_pnjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_pnjob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	isPlayerWhitelisted = refreshPlayerWhitelisted()
	Citizen.Wait(5000)
	TriggerServerEvent('esx_pnjob:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'pn' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_pnjob:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_pnjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_pnjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job and PlayerData.job.name == 'pn' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_pnjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_pnjob:handcuff')
AddEventHandler('esx_pnjob:handcuff', function()
...
RegisterNetEvent('esx_pnjob:unrestrain')
AddEventHandler('esx_pnjob:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('esx_pnjob:drag')
AddEventHandler('esx_pnjob:drag', function(copId)
...
RegisterNetEvent('esx_pnjob:putInVehicle')
AddEventHandler('esx_pnjob:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_pnjob:OutVehicle')
AddEventHandler('esx_pnjob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.PnStations) do
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'pn' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_pnjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_pnjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_pnjob:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_pnjob:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'pn' then
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'pn' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'PN_actions') then
			if Config.MaxInService == -1 then
...
AddEventHandler('renfort:setBlip', function(coords, raison)
	if raison == 'petit' then
		...
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Demande de renfort', 'Demande de renfort demandé.\nRéponse: ~g~CODE-2\n~w~Importance: ~g~Légère.', 'CHAR_CALL911', 8)
		Wait(1000)
...
AddEventHandler('pn:InfoService', function(service, nom)
	if service == 'prise' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Prise de service', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-8\n~w~Information: ~g~Prise de service.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'fin' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Fin de service', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-10\n~w~Information: ~g~Fin de service.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'pause' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Pause de service', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-6\n~w~Information: ~g~Pause de service.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'standby' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Mise en standby', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-12\n~w~Information: ~g~Standby, en attente de dispatch.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'control' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Control routier', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-48\n~w~Information: ~g~Control routier en cours.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'refus' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Refus d\'obtempérer', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-30\n~w~Information: ~g~Refus d\'obtempérer / Délit de fuite en cours.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'arrestation' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('PN INFORMATIONS', '~b~Arrestation en cours', 'Agent: ~g~'..nom..'\n~w~Code: ~g~10-31\n~w~Information: ~g~Arrestation en cours.', 'CHAR_CALL911', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	end
end)
...
RegisterNetEvent('esx_pnjob:updateBlip')
AddEventHandler('esx_pnjob:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_pnjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_pnjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_pnjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'pn')
...
RegisterNetEvent('esx_pnjob:getarrested')
AddEventHandler('esx_pnjob:getarrested', function(playerheading, playercoords, playerlocation)
...
RegisterNetEvent('esx_pnjob:doarrested')
AddEventHandler('esx_pnjob:doarrested', function()
	Citizen.Wait(250)
	loadanimdict('mp_arrest_paired')
...
RegisterNetEvent('esx_pnjob:douncuffing')
AddEventHandler('esx_pnjob:douncuffing', function()
	Citizen.Wait(250)
	loadanimdict('mp_arresting')
...
RegisterNetEvent('esx_pnjob:getuncuffed')
AddEventHandler('esx_pnjob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	z = z - 1.0
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
